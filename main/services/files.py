...
from main.obj.errors import OutboundNetworkError
...
async def write_milvus_record(db_records: List[DBRecord]) -> int:
    ...
    # client.flush(collection_name=env.MILVUS_COLLECTION)
...
async def store_chunks(chunk_list: List[Chunk]) -> int:
    ...
    try:
        ...
        # database records.
        db_records = [
            DBRecord(**chunk.model_dump(mode="python"), 
                     vector=emb, 
                     chunk_hash=xxh64(chunk.text).hexdigest())
